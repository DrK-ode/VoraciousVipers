...
ArenaScene::PlayerData ArenaScene::read_player_conf(size_t player) {
  ...
  auto mouse_enabled =
    game_resources().options_service().option_boolean(base_name + "useMouse");
  if (keys.size() != 3)
    throw std::runtime_error("Wrong number of keys in player configuration.");
...
ArenaScene::ArenaScene(GameResources& game_resources)
  ...
  std::ranges::for_each(
    std::views::iota(0) | std::views::take(number_of_players) |
      std::views::transform(
        [this](auto player) { return read_player_conf(player); }),
    [&player_data](auto player_conf) { player_data.push_back(player_conf); });
  ...
  std::ranges::for_each(
    status_bar_views | std::views::enumerate, [&](auto view_tuple) {
      auto&& [i, view] = view_tuple;
      view.setSize(status_bar_size);
      view.setCenter(0.5 * status_bar_size.x, 0.5 * status_bar_size.y);
      view.setViewport(sf::FloatRect({i * status_bar_relative_size.x, 0.},
                                     status_bar_relative_size));
    });
...
void ArenaScene::add_players(std::vector<PlayerData>& player_data,
  ...
  std::ranges::for_each(
    std::views::zip(player_data, playerViews), [&, this](auto player_tuple) {
      auto&& [data, view] = player_tuple;
      std::unique_ptr<Controller> controller;
      if (data.mouse_enabled) {
        controller = std::make_unique<MouseController>(game_resources());
      } else {
        KeyboardController::KeyboardControls keys;
        keys.left = sf::Keyboard::Scancode(data.keys[0]);
        keys.right = sf::Keyboard::Scancode(data.keys[1]);
        keys.boost = sf::Keyboard::Scancode(data.keys[2]);
      ...
      add_player(data, std::move(controller), std::move(viper), view);
    });
}
...
Player* ArenaScene::add_player(const PlayerData& data,
  ...
  auto& player = _players.emplace_back(
    make_unique<Player>(data.name, std::move(controller), std::move(viper)));
...
void ArenaScene::delete_player(Player* player) {
  ...
  _players.erase(std::find_if(_players.begin(), _players.end(),
...
std::unique_ptr<Viper> ArenaScene::add_viper(
  ...
  auto viper =
    std::make_unique<Viper>(std::move(viper_configuration),
                            starting_area.anchor(), starting_area.angle(), 0.5);
...
PlayerPanel* ArenaScene::find_player_panel(const Player* player) const {
  auto iter = std::find_if(_player_panels.begin(), _player_panels.end(),
...
void ArenaScene::find_free_space_for(
  ...
  for (int _ : std::views::iota(0, max_attempts)) {
    ...
    if (std::ranges::any_of(exclusion_zones, [&shape](auto& zone) {
          return zone.overlap(shape);
        })) {
      continue;
    }
...
void ArenaScene::dispense_food() {
  while (_food.size() < 2) {
    ...
    double food_radius =
      std::sqrt(Random::random_double(smallest * smallest, largest * largest));
...
void ArenaScene::handle_viper_food_collision(Viper* viper,
  ...
  auto& flyingScore = _flying_scores.emplace_back(std::make_unique<FlyingScore>(
...
void ArenaScene::handle_viper_viper_collision(Viper* collider_viper,
  ...
  if (collider_viper == collidee_viper) {
    ...
    if (segment_distance > 1) {
      log_debug(collider_viper->name(), " was killed by a collision with ",
