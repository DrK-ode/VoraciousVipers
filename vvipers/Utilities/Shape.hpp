...
class BoundingBox {
  public:
    BoundingBox(double x1, double x2, double y1, double y2) : x_min(x1), x_max(x2), y_min(y1), y_max(y2) {}
    ...
    bool overlap(const BoundingBox& other) const {
        return !(this->x_min > other.x_max || this->x_max < other.x_min || this->y_min > other.y_max ||
...
class Shape {
  public:
    ...
    virtual std::tuple<double, double> projection_on_vector(const Vec2&) const = 0;
...
class Circle : public Shape {
  public:
    Circle(const Vec2& center, double radius) : Shape(ShapeType::Circle), _center(center), _radius(radius) {}
    BoundingBox bounding_box() const override {
        return {_center.x - _radius, _center.x + _radius, _center.y - _radius, _center.y + _radius};
    }
