    ...
    switch (other.type()) {
        case ShapeType::Circle: {
            ...
            for (auto& axis : axes) {
                ...
                auto [circ_min, circ_max] = other_circle.projection_on_vector(axis);
        ...
        case ShapeType::Polygon: {
            const Polygon& other_polygon = reinterpret_cast<const Polygon&>(other);
            const auto axes = this->_corners.size() < other_polygon._corners.size()
                                  ? this->normal_vectors()
...
void Polygon::rotate(double rads) {
    for (Vec2& corner : _corners){
        corner = _center + (corner - _center).rotate(rads);
    }
}
